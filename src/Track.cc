...
double Tracking::Track::residual(const TrackHit& hit, bool print)
{
    ...
    Measurement_t delta= pos-this->statusAt((this->tLayout_->layers_[hit.layer_].position_(2))); //calculation of the residual
    if (print)
    {
        std::cout << "HIT POS " << pos << std::endl;
        std::cout << "TRACK STATUS " << this->statusAt((this->tLayout_->layers_[hit.layer_].position_(2))) << std::endl;
  ...
  : TNamed()
{
    ...
    for (auto& layer: layers)
    {
        ...
        Tracking::TelescopeLayer aLayer(layerPos,zRotationAngle);
